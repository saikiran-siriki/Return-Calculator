...
export default function Calculator({assetData}) {
    ...
    const [duration, setDuration] = useState([{name: '1 Year', active: true, days: 365},{name: '2 Year', active: false, days: 730},{name: '3 Year', active: false, days: 1095}])
    ...
    useEffect(()=>{
        ...
        const todayAsssetValue = assetData.at(-1)[1]
        const filteredAssetAccumulatedVal = filteredAssetDataArr.reduce((a, b, index: number)=>{console.log( index,filteredDcaDays, index%filteredDcaDays===0,a+b[1]);if(index%filteredDcaDays===0) {return a+(investmentPerTimeFrame/b[1])} else {return a}},0)
        setPortfolioValue((filteredAssetAccumulatedVal*todayAsssetValue).toFixed(2))
    ...
    function displayProfitLoss() {
        ...
        return <span className={`${PnL>=0?'profit':'loss'}`}>{PnL}</span>
    }
    ...
    return (<>
        ...
        <div className={styles.slider_container}>
            <Slider defaultValue={1} aria-label="Default" valueLabelDisplay="auto" onChangeCommitted={onSliderChange} max={1000} min={5} step={10} color="secondary" marks/>
        </div>
